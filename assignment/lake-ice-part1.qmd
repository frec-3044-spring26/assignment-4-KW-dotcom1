...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_line() + geom_point() + geom_line() +
  labs(title = "Sunapee Lake Ice Off Day of Year Over Time", x = "Year", 
       y = "Day of the Year")

```

# How does ice cover change over time at Lake Sunapee?
The ice-off coverage seems to be trending downwards in relation to the days of the yearfor Lake Sunapee, signifying increasing global temperatures raises. This would lead to earlier Spring melts, or hotter days earlier in the transition from Spring to Summer.
...
Considering the earlier Spring melting days for snow coverage, I would expect the signage for the slope to be "-", negative.
...
The direction of the slope is negative. This was expected given the circumstances of question 2

# Is our model any good?

Yes, our model is good, and is statistically significant. The slope is negative and shows that Lake Sunapee, for every 1-year increase over time, is experiencing ice-off melts .11 days earlier year-over-year.
...
The R2 is 0.2055. The relationship is relatively weak. The R2 means that the variable year only explains about 20% of the variation in ice_off_day for Lake Sunapee. This means that the remaining 80% of the variation is probably due to other reasons or "noise" such as specific yearly weather patterns, snowfall, or temp spikes. THe relationship is statistically significant though, but the year from R2 is not a strong predictor of the exact day ice will menlt in any single year
...
The histogram is centered on 0 (the dashed line), meaning the model is not biased, but the residuals are spread out between -30 and +20 days, letting us know that although there is significant long-term trends, the model has low precision for predicting a specific year's ice off day due to the high variability. 
...
ggplot(sunapee, aes(x = resid)) +
  ...
  labs(title = "Distribution of Residuals for Lake Sunapee",
       x = "Residual (in Days)",
       y = "Frequency")
summary(fit)
...
The probability of observing this slope value assuming the actual slope is 0 is 
0.00000025%. It is not large enough for us to believe the slope could be zero. The t-value of -6.331 let's us know that the slope is over 6 standard deviations away from 0.
...
7/0.10602 = 66.02, so it would take approximately 66 years for the ice off date to shift by one full week. This is meaningful as ecological life depends on the changing of the seasons, and the transitionary states therefrom. For example, a lot of wildlife such as fish, spawn when the ice leaves and water warms up. If ice leaves a week earlier, it can cause a mismatch of fish spawning without their associated prey being around. For humans, a week of less ice could sevrely impact the snow mobile/skiing resorts in their annual revenue streams. 
...
**Answer 9:**
I am comfortable using this model to help me identify and describe trends over time, just not very specific predictions.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  geom_abline(intercept = 318.94856, 
              ...
              color = "blue", 
              linewidth = 1) + 
  labs(title = "Lake Sunapee Ice-Off Trend",
       subtitle = "Blue line represents linear regression",
       x = "Year",
...
lower <- quantile(sunapee$resid, 0.05)
upper <- quantile(sunapee$resid, 0.95)


lower
#output is 5% , -19.42732 
upper
# output is 95%, 14.15137 

final_lower <- prediction_2040 + lower

final_upper <- prediction_2040 + upper

final_lower
#output is 5%, 83.24044 

final_upper
#output is 95%, 116.8191 

width <- final_upper - final_lower
width

#Output is 95%, 33.57868 
...
early_sunapee <- sunapee |> filter(year < 1970)
late_sunapee  <- sunapee |> filter(year >= 1970)

fit_early <- lm(ice_off_doy ~ year, data = early_sunapee)
fit_late  <- lm(ice_off_doy ~ year, data = late_sunapee)

summary(fit_early)$coefficients
summary(fit_late)$coefficients


ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_smooth(data = early_sunapee, method = "lm", se = FALSE, color = "blue") +
  geom_smooth(data = late_sunapee, method = "lm", se = FALSE, color = "red") +
  labs(title = "Comparison of Ice Off Trends: Pre vs. Post 1970",
       subtitle = "Blue = 1869-1969 | Red = 1970-2014",
...
Yes, the ice off day of the year is changing more quickly, as shown by the graph comparision of pre-1970 and post-1970. The latter trendline is steeper, indicating a more drastic decrease comparatively.
